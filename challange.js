let elizabethSanger = {
  ...
  const donationFormStringBuilder = () => {
      const newString = `<a>href="https://${elizabethSanger.donationFormUrl}"register to vote</a>`
      printToDom(newString, 'donationFormUrl');
  };
  ...
  const statementsStringBuilder = () => {
      ...
      for(let i=0; i<elizabethSanger.statements.length; i++){
         newString += `<div class="statement"><h3>abethSanger.statements[i].statement;</h3></div>`;
